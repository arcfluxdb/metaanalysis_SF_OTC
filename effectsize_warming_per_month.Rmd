...
=======
  geom_bar(aes(x=month, fill = treatment),stat = "count") + 
>>>>>>> 45ddc6b09afa3dbbe20854a2c5260a737beb8235
  theme_classic()
```
 first up splittinf the data into the different treatments
```{r}
working_data %>% 
  group_by(site_id, year) %>% 
  dplyr::summarise(treatment = paste(unique(treatment), collapse = ", ")) %>% 
  view()

working_data %>% 
  group_by(site_id, year) %>% 
  dplyr::summarise(ctl = sum(treatment == "CTL",na.rm=T),
                   otc = sum(treatment == "OTC",na.rm=T),
                   sf = sum(treatment == "SNOWFENCE",na.rm=T),
                   otc_sf = sum(treatment == "OTCxSNOWFENCE",na.rm=T)
                   ) %>% 
  view()
...
ggplot(otc_effect_sizes)+
  ...
  geom_smooth(aes(x=month,y=yi_otc),method = "lm", se=F)+
  geom_jitter(aes(x=as.factor(month),y=yi_otc,color=site_id),width = 0.1)+
  theme_minimal()
...
ggplot(sf_effect_sizes)+
  ...
  geom_smooth(aes(x=month,y=yi_sf),method = "lm", se=F)+
  geom_jitter(aes(x=as.factor(month),y=yi_sf,color=site_id),width = 0.1)+
  theme_minimal()
```
...
ggplot(otc_sf_effect_sizes)+
  ...
  geom_smooth(aes(x=month,y=yi_otc_sf),method = "lm", se=F)+
  geom_jitter(aes(x=as.factor(month),y=yi_otc_sf,color=site_id),width = 0.1)+
  theme_minimal()
```
...
ggplot(stacked_effect)+
  ...
  geom_jitter(aes(x=as.factor(month),y=effectsize,color=site_id),width = 0.1)+
  theme_classic()
